...
void Server::init() {
  ...
  _loop = new EventLoop();
  _serverSocket->bind(&_serverAddr);
...
void Server::loop() {
  ...
  Channel servChannel(_serverSocket->getFd());
  servChannel.prepareEvents();
  // 4. 对于 服务socket的回调函数, 是新增一个连接
  servChannel.setCallback([&]() { newConnection(_serverSocket, _loop); });
  _loop->updateChannel(&servChannel);
